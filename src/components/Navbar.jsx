...
const Navbar = () => {
    ...
    const NavActions = (
        <ul className="flex flex-col md:flex-row items-center gap-6 md:gap-10 bg-white/10 dark:bg-slate-800/20 backdrop-blur-md rounded-2xl md:rounded-full border border-white/20 dark:border-slate-600/30 shadow-md px-6 py-4 md:px-6 md:py-2">
            <li>
                <button
                    className={`flex items-center gap-2 px-3 py-1 rounded-full font-semibold hover:bg-gradient-to-r hover:from-indigo-400 hover:to-purple-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 ${isActive("/aboutme") ? "text-indigo-300 font-bold" : "text-white"}`}
                    aria-label="About Me"
            ...
            <li>
                <button
                    className={`flex items-center gap-2 px-3 py-1 rounded-full font-semibold hover:bg-gradient-to-r hover:from-indigo-400 hover:to-purple-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 ${isActive("/offers") ? "text-indigo-300 font-bold" : "text-white"}`}
                    aria-label="Offers"
                    // onClick for offers if you have a route
                >
                    <PercentIcon className="text-base" />
                    <span className="sm:inline">Offers</span>
                </button>
            </li>
            <li>
                <Link
                    to="/cart"
                    className={`flex items-center gap-2 px-3 py-1 rounded-full font-semibold hover:bg-gradient-to-r hover:from-indigo-400 hover:to-purple-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 ${isActive("/cart") ? "text-indigo-300 font-bold" : "text-white"}`}
                    aria-label="Cart"
            ...
            <li>
                <button
                    className={`flex items-center gap-2 px-3 py-1 rounded-full font-semibold hover:bg-gradient-to-r hover:from-indigo-400 hover:to-purple-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 text-white`}
                    aria-label="Toggle theme"
                    onClick={() => { toggleTheme(); setMenuOpen(false); }}
                >
                    {isDarkMode ? <LightModeIcon className="text-base" /> : <DarkModeIcon className="text-base" />}
                    <span className="sm:inline">{isDarkMode ? "Light" : "Dark"}</span>
                </button>
            </li>
            <li>
                <button
                ...
                >
                    ...
                    <span className="sm:inline">{loginToken ? "Logout" : "Login"}</span>
                </button>
            </li>
        </ul>
    );
    ...
    const LocationSearch = (
        <div className="flex items-center bg-white/10 dark:bg-slate-800/20 backdrop-blur-md rounded-2xl md:rounded-full border border-white/20 dark:border-slate-600/30 shadow-md px-4 py-3 md:px-4 md:py-2 gap-2 mt-4 md:mt-0">
            <LocationOnOutlinedIcon className="text-indigo-300 dark:text-indigo-400 text-lg" />
            {isLocationEditable ? (
                ...
                <button
                    className="text-xs md:text-sm text-white font-bold cursor-pointer hover:text-indigo-300 transition-all duration-300 hover:scale-105"
                    onClick={() => setIsLocationEditable(true)}
                    aria-label="Edit location"
                >{customLocation}</button>
            )}
            <div className="flex items-center gap-1 ml-3">
                <SearchIcon className="text-lg text-indigo-300 dark:text-indigo-400" />
    ...
    return (
        <div className="parent w-full bg-gradient-to-r from-slate-800 via-indigo-800 to-purple-800 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 shadow-2xl dark:shadow-slate-900/50 border-b-4 border-indigo-400/50 dark:border-slate-600/50 transition-all duration-300 px-4 md:px-8 py-2">
            <div className="flex items-center justify-between w-full">
                {/* Logo and Brand */}
                <Link to="/" className="flex items-center cursor-pointer hover:scale-105 transition-all duration-300 group">
                    <img
                        className="logo ml-2 md:ml-10 w-16 h-16 object-contain rounded-xl shadow-md bg-white"
                    ...
                    style={{ 
                        fontFamily: 'Dancing Script, Noto Nastaliq Urdu, cursive', 
                ...
                <div className="hidden md:flex flex-1 justify-end items-center">
                    {NavActions}
                    <div className="ml-10">{LocationSearch}</div>
                </div>
            </div>
